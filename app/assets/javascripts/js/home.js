...
var watchdogreportUrl = '/getwatchdogreport'
...
var campaignAdgroupBaiduUpdateUrl='/baidu/updateadgroup';
var campaignAdgroupShenmaUpdateUrl='/shenmas/updateadgroup';
...
var campaignAdsBaiduUpdateUrl='/baidu/updatead';
var campaignAdsShenmaUpdateUrl='/shenmas/updatead';
...
var campaignKeywordBaiduUpdateUrl='/baidu/updatekeyword';
var campaignKeywordShenmaUpdateUrl='/shenmas/updatekeyword';
...
function setDate(range){
	...
	if(range=='Yesterday'){
		...
		start_date=new Date(d.getFullYear(), d.getMonth()-3, 1);
		end_date=new Date(d.getFullYear(), d.getMonth(), 1,-1);
...
function genLineChart(greenDataIndex, orangeDataIndex){
	$("#line-chart").remove();
  	...
  	$('#line-chart-container1').append('<canvas class="line-chart" id="line-chart"><canvas>');
	...
	if(greenDataIndex=='ctr' || orangeDataIndex=='ctr'){
		var lineGraphOptions = {
			bezierCurve : false,
			scaleLabel: "<%=accounting.formatNumber(value)%>%",
			multiTooltipTemplate: "<%= accounting.formatNumber(value) %>"
...
function genLineChart2(greenDataIndex, orangeDataIndex){
	$("#line-chart2").remove();
  	$('#line-chart-container2').append('<canvas class="line-chart" id="line-chart2"><canvas>');
...
function autoSelect(id){
	...
	sel.removeAllRanges();
	sel.addRange(range);
  document.execCommand("copy");*/ // leon copy to clipboard
}
// tracking end
...
// quick add
function showQuickAddContentOuter(){
	
	
	prepareOpenExtendContent("quickadd-content-outer",function(){
		$("#quickadd-content-outer").slideDown();
	});
...
function selectRuleField(element){
	...
	if($(element).val()=="account_name" || $(element).val()=="adgroup_name" || $(element).val()=="campaign_name" || $(element).val()=="desc_1" || $(element).val()=="desc_2" || $(element).val()=="display_url" || $(element).val()=="m_display_url" || $(element).val()=="final_url" || $(element).val()=="m_final_url" || $(element).val()=="headline"){
		...
		fieldHtml+='<input type="text" name="field_value" class="form_field">%';
	}else if($(element).val()=="avg_pos" || $(element).val()=="clicks" || $(element).val()=="conv" || $(element).val()=="impr" || $(element).val()=="roi"){
		// datatype = number
...
function resetRule(){
	...
	$("input[type=text].form_field").val('');
}
// advanced filter
...
function submitEditStatus(){
	if(editStatusValid){
		...
		if(valid){
			...
			}else if($("#campaign_type").val()=="baidu"){
				var url=editBaiduUrl;
			}else if($("#campaign_type").val()=="shenma"){
				var url=editShenmaUrl;
			}
...
function submitAdgroupSync(){
	...
	var network_id = $('input[name^=network_id]').val()
	...
	if(id_length > 0){
		$.ajax({
			url: "/adgroup_api_sync",
			type: 'POST',
			data: {"id":ids,"campaign_type":campaign_type,"campaign_id":campaign_id,"network_id":network_id},
			success: function(data,status,xhr){
				$(".loading_container").hide();
				if(data.status=='true'){
		...
		$('#edit_error_modal .modal_title').html("Please Select Adgroup.");
		$('#edit_error_modal').modal('show');
	}
...
function submitCampaignSync(){
	...
	if(id_length > 0){
		...
		$('#edit_error_modal .modal_title').html("Please Select Campaigns.");
		$('#edit_error_modal').modal('show');
	}
	// console.log(ids);
}
...
function submitSync(){
	...
	if(id_length > 0){
		...
		for (key in ids) {
			...
			if(id_array[1] == "sogou"){
				var url = "http://china.adeqo.com:85/sogous/resetdlfile?id="+id_array[0];
				// var url = "http://china.adeqo.com:83/test";

			}else if(id_array[1] == "360"){
				var url = "http://china.adeqo.com:85/threesixties/resetdlfile?id="+id_array[0];
				// var url = "http://china.adeqo.com:83/test2";
			}else if(id_array[1] == "baidu"){
				var url = "http://china.adeqo.com:85/baidu/resetdlfile?id="+id_array[0];
				// var url = "http://china.adeqo.com:83/test2";
			}
...
function showAddKeyword(){
	...
	// if($("input[name^=id]:checked").length>0){
		...
		// $("input[name^=id]:checked").each(function(){
			// $("#findreplace_hidden_checkbox").append('<input type="checkbox" name="item_id[]" value="'+this.value+'" checked />');
		// })
		prepareOpenExtendContent("add-keyword-content-outer",function(){
			$("#add-keyword-content-outer").slideDown();
		});
// 
		// $("input[name^=id]").attr("disabled", true);
		// $("#select_all").attr("disabled", true);
...
function hideQuickContentOuter(){
	$("#quickadd-content-outer").slideUp();
	$('textarea#quickaddinput').val('');
	$("input[name^=id]").removeAttr("disabled");
	// $("#select_all").removeAttr("disabled");
}
