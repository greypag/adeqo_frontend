...
$(document).ready(function(){
	...
	$(".custom-datepicker-field").datepicker({
		format: 'd MM yyyy',
		autoclose: true,
		orientation: "top",
		endDate: '0d'
	...
	$(".custom-datepicker-field").each(function(i,o){
		$(o).datepicker("setDate", new Date());
		$(o).datepicker().on("changeDate", function(e){
			$("#custom_start_date").datepicker("setEndDate",$("#custom_end_date").val());
			$("#custom_end_date").datepicker("setStartDate",$("#custom_start_date").val());
			setDate("Date Range");
		})
	});
		
	
	$("[name=custom_start_date]").change(function(){
		$("[name=start_date]").val($("[name=custom_start_date]").val());
	});
	
	$("[name=custom_end_date]").change(function(){
		$("[name=end_date]").val($("[name=custom_end_date]").val())
	});	
	...
	$("[name=page]").change(function(){
		...
		if($("[name=page]").val() == "Campaign"){
			...
			$(".form_field_require").append( '<option class="campaign" value="roi">ROI</option>' );				
		}
		...
		if($("[name=page]").val() == "Ad Groups"){
			...
			$(".form_field_require").append( '<option class="adgroup" value="roi">ROI</option>' );
		}
		...
		if($("[name=page]").val() == "Keywords"){
			...
			$(".form_field_require").append( '<option class="keyword" value="roi">ROI</option>' );
		}
